...
export const handler = async (event) => {
  ...
  try {
    ...
    // Without MaxKeys, S3 returns up to 1000 keys per page -- which means
    // the entire KB bucket comes back in a single response and the admin
    // table renders all rows at once. Cap the page size so the table
    // paginates and the user can step through with Previous/Next.
    const PAGE_SIZE = 50;
