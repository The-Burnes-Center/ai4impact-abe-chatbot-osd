...
When the first \`query_db\` for a contract returns chunks from only one file but the user's question is about a topic the CUG may not cover (e.g. original procurement timeline, bid/release dates, evaluation criteria, solicitation Q&A, addendum-specific terms), consult \`{{metadata_json}}\` (the document inventory below) for sibling documents tied to the same contract ID and either re-run \`query_db\` with more targeted terms or call \`retrieve_full_document\` on the relevant sibling. Treat the document inventory as the authoritative list of what's available — if a sibling document exists for the contract ID, do not stop at the CUG.
