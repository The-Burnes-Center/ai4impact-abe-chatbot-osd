...
async function summarizeHistory(history, connectionId, opts = {}) {
  ...
  try {
    await wsConnectionClient.send(new PostToConnectionCommand({
      ...
      Data: "!<|STATUS|>!Compacting earlier conversation to keep going\u2026"
    }));
  } catch (statusErr) {
  ...
  try {
    ...
    while (!stopLoop) {
      ...
      if (estimatedTokens > MAX_ESTIMATED_TOKENS) {
        ...
        const inlineNote = "I've compacted this conversation as much as I can while preserving the important parts, but it's still too large for me to continue here. To keep going on a related question, please click \"+ New chat\" in the sidebar to start a fresh conversation.";
        try {
