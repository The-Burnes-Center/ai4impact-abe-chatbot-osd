...
const getUserResponse = async (id, requestJSON) => {
  try {
    ...
    // Keep last 6 exchange pairs (12 messages + current prompt = 13 messages in context).
    // Result-size caps (capToolResultSize) keep per-turn token usage bounded.
    let claude = new ClaudeModel();
    let lastFiveMessages = chatHistory.slice(-6);
    ...
    while (!stopLoop) {
      ...
      if (estimatedTokens > MAX_ESTIMATED_TOKENS) {
        console.error(`Context too large: ~${estimatedTokens} tokens. Attempting to trim oversized tool results.`);
        let trimmed = false;
        for (const msg of history) {
          ...
          for (const block of msg.content) {
            if (block.type === "document" && block.source?.data) {
              ...
              if (dataLen > MAX_TOOL_RESULT_CHARS) {
                try {
                  ...
                  block.source.data = JSON.stringify({
                    ...
                    _note: `Full results (${parsed.total_matches} matches) exceeded context limits. Use count_unique, group_by, or narrower filters.`
                  });
                  trimmed = true;
                } catch (_) {
