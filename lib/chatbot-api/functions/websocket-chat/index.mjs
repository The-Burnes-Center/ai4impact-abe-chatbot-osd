...
function buildExcelIndexTool(indexes) {
  ...
  return {
    ...
    description: `Query structured Excel-based data indexes. Available indexes:\n\n${indexDescriptions}\n\nUse free_text for broad search across all columns. Use filters for column-specific matching (keys are exact column names from above). Matching is punctuation-insensitive.\n\nResponse includes: rows (array), total_matches (row count), returned (rows returned), and unique_vendors (distinct vendor count when Vendor_Name column exists). For "how many vendors" questions, ALWAYS use unique_vendors — NOT total_matches, since one vendor may have multiple rows.`,
    input_schema: {
      type: "object",
      properties: {
