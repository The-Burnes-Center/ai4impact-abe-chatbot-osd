...
const getUserResponse = async (id, requestJSON) => {
  try {
    // ⭐ SEND IMMEDIATE ACKNOWLEDGMENT TO PREVENT FRONTEND TIMEOUT
    console.log("Sending immediate acknowledgment to frontend");
    const ackParams = {
      ConnectionId: id,
      Data: "PROCESSING_STARTED"
    };
    await wsConnectionClient.send(new PostToConnectionCommand(ackParams));
...
// ⭐ UPDATED HANDLER WITH ASYNC PROCESSING + CONTEXT CONTROL
export const handler = async (event, context) => {
  // Keep Lambda alive for async operations
  ...
  if (event.requestContext) {    
    ...
    switch (routeKey) {
      ...
      case "getChatbotResponse":
        ...
        // Start async processing - Lambda will stay alive due to context setting
        getUserResponse(connectionId, body).catch(error => {
          console.error("Async processing failed:", error);
          // Send error to client
          wsConnectionClient.send(new PostToConnectionCommand({
