          ...
          if (parsedChunk) {                      
            ...
            if (parsedChunk.stop_reason) {
              if (parsedChunk.stop_reason == "tool_use") {
                ...
                // end_turn: flush the final answer to the client.
                // Text from intermediate iterations (stop_reason: tool_use) is
                // never sent — only the answer produced after all tools complete.
            ...
            if (usingTool) {
              ...
              if  (assemblingInput && !skipChunk) {
                ...
                // Always buffer — never stream mid-iteration.
                // If this iteration ends with tool_use, the text is reasoning
                // and must not reach the user. If it ends with end_turn,
                // the text is the final answer and is flushed in the
                // stop_reason handler above.
