 ...
 */
async function loadAuthoritativeSession(userId, sessionId) {
  try {
 ...
 *   session_id, display_name, agency. Note: any client-supplied chatHistory is
 *   ignored; prior turns are loaded authoritatively from DynamoDB instead.
...
const getUserResponse = async (id, requestJSON, authorizedUserId) => {
  try {
    ...
    // SECURITY: Never trust data.chatHistory from the WebSocket frame -- it is
    // attacker-controlled and was previously fed straight into the model,
    // letting a caller forge prior assistant turns to bypass the system prompt
    // or inject false context. Reconstruct the conversation only from the
    // session row stored under the JWT-verified userId + sessionId. The client
    // value is ignored entirely (kept as a display hint on the frontend only).
    const { history: storedHistory, contextSummary: storedSummary } =
      await loadAuthoritativeSession(userId, sessionId);
    const isFirstTurn = storedHistory.length === 0;
    ...
    let lastMessages = storedHistory.slice(-12);
     ...
     */
    const userGoal =
      storedHistory.length > 0 && storedHistory[0]?.user
        ? String(storedHistory[0].user)
        : userMessage;

    let history = claude.assembleHistory(lastMessages, userMessage)

    // Prepend the persisted context summary (from a prior compression) so the
    // model retains earlier context that was compacted out of stored history.
    // Loaded from DynamoDB alongside the authoritative history above.
    if (storedSummary) {
      history = [
        { role: "user", content: [{ type: "text", text: `[CONVERSATION SUMMARY]\n${storedSummary}` }] },
        { role: "assistant", content: [{ type: "text", text: "Understood. I have the context from our earlier conversation and will continue naturally." }] },
            ...
            if (parsedChunk.stop_reason === "tool_use") {
              ...
              for (const tc of toolCalls) {
                ...
                if (tc.name === "query_db") {
                  ...
                  try {
                    await safeSend({
                      ConnectionId: id, Data: filenameContains
                        ? `!<|STATUS|>!Fetching document inventory for "${truncate(filenameContains)}"...`
                  ...
                  toolResults.push({ toolId: tc.id, content: metadata ? capToolResultSize(JSON.stringify(metadata)) : "No metadata available.", documentBlocks: [] });
    ...
    const turnIndex = storedHistory.length + 1;
    try {
