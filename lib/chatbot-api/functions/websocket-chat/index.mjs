 ...
 * - "<!SESSION_FULL!>: ...": terminal error indicating the conversation cannot
 *   continue because the model context is exhausted; the client should auto-
 *   redirect the user to a fresh session and carry over the unsent question.
  ...
  try {
    ...
    while (!stopLoop) {
      ...
      if (estimatedTokens > MAX_ESTIMATED_TOKENS) {
        ...
        if (estimatedTokens > MAX_ESTIMATED_TOKENS || !trimmed) {
          ...
          try {
            await safeSend({
              ConnectionId: id,
              Data: "<!SESSION_FULL!>: This conversation has reached its memory limit. Starting a fresh conversation so we can keep going."
            });
          } catch (sendErr) {
      ...
      try {
        ...
        // (e.g., "Input is too long for requested model"). Surface SESSION_FULL so
        // the client redirects to a fresh session instead of showing a generic error.
        ...
        try {
          await safeSend({
            ConnectionId: id,
            Data: isContextOverflow
              ? "<!SESSION_FULL!>: This conversation has reached its memory limit. Starting a fresh conversation so we can keep going."
