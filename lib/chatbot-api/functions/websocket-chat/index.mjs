 ...
 *
 * Markers are placed at the END of the cited span so they read naturally:
 *   "The contract term is December 2025.[1]"
 * rather than at the start:
 *   "[1]The contract term is December 2025."
 *
 * The insertion offset is resolved in order of preference:
 *   1. citation.end_char_index  (exact end from Bedrock)
 *   2. textOffset + citation.cited_text.length
 *   3. next sentence boundary after textOffset
 *   4. textOffset (original fallback)
 */
function insertCitationMarkers(text, citations, docIndexMap, allSources) {
  ...
  for (const { textOffset, citation } of citations) {
    ...
    if (citation.end_char_index != null && citation.end_char_index <= text.length) {
      insertAt = citation.end_char_index;
    } else if (citation.cited_text) {
      insertAt = Math.min(textOffset + citation.cited_text.length, text.length);
    } else {
      // Find end of current sentence/clause from textOffset
      const rest = text.slice(textOffset);
      const sentEnd = rest.search(/[.!?;]\s|\n/);
      insertAt = sentEnd >= 0 ? textOffset + sentEnd + 1 : text.length;
    }

    insertions.push({ offset: insertAt, chunkIndex: source.chunkIndex });
  }

  // Sort descending by offset so earlier positions aren't shifted
  insertions.sort((a, b) => b.offset - a.offset);

  for (const { offset, chunkIndex } of insertions) {
    const marker = `[${chunkIndex}]`;
 ...
 */
function renumberCitations(text, allSources) {
