...
const getUserResponse = async (id, requestJSON, authorizedUserId) => {
  try {
    ...
    /**
     * Aborts the in-flight Bedrock streaming call when the client disconnects
     * (presses stop / closes the tab), so we stop generating — and paying for —
      ...
      try {
        ...
        if (err.name === 'GoneException' || err.$metadata?.httpStatusCode === 410) {
          if (!connectionGone) {
            ...
            connectionGone = true;
            // Cancel the in-flight Bedrock stream so we stop generating (and
            // paying for) tokens the client will never receive.
    ...
    while (!stopLoop) {
      ...
      // Client left (pressed stop / closed tab) — don't spend another Bedrock
      // call; the abort signal also tears down any in-flight stream.
      if (connectionGone) {
        stopLoop = true;
        break;
      }
      ...
      try {
        ...
        for await (const event of stream) {
          ...
          // Heartbeat: keep the WebSocket alive during long answer generation.
          // The frontend resets its inactivity timer on every STATUS frame.
          if (Date.now() - lastStatusSentAt > 20_000) {
        ...
        // Client disconnected (pressed stop): the Bedrock stream was aborted on
        // purpose. Stop cleanly and keep whatever partial answer we have — never
        // log it as a failure or try to send an error frame.
        if (connectionGone || error?.name === 'AbortError') {
    ...
    // Client disconnected (pressed stop / closed the tab): the Bedrock stream is
    // already aborted and the loop broken. Treat it as a clean cancel — persist
    // nothing (a deliberate stop must never reappear on reload) and skip the
    // finalize sends that would only hit a dead connection.
    if (connectionGone) {
