...
async function loadIndexMetadata() {
  ...
  try {
    ...
    for (const item of resp.Items || []) {
      ...
      indexes.push({ index_name: indexName, display_name: displayName, columns, row_count: rowCount });
    }
  ...
  return {
    ...
    input_schema: {
      ...
      properties: {
        ...
        count_unique: { type: "string", description: "Column name to count distinct values for. Returns unique_count in the response. Use when the user asks 'how many unique X' — e.g. count_unique: 'Vendor_Name' for distinct vendors, 'Contract_ID' for distinct contracts, 'Agency' for distinct agencies, etc." },
