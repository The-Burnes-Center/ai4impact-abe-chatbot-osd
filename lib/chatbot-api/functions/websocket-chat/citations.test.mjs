...
describe("snapToSentenceEnd", () => {
  ...
  it("slides citation inside the cell when the row would otherwise close with a glued marker", () => {
    // The header row ends at offset = length of "| Field | Details |" before the newline.
    const row = "| Field | Details |";
    const text = row + "\n|---|---|\n| Contract | CLT09 |\n";
    // With rawOffset somewhere mid-row, snapToSentenceEnd would naturally
    // walk to the '\n' at the end. We need it to slide back before the
    // trailing pipe so the table stays parseable.
    const offset = snapToSentenceEnd(text, 8);
    // Should land on or before the closing pipe, never on the newline that
    // immediately follows it.
    expect(text[offset]).not.toBe("|");
    expect(text[offset - 1]).not.toBe("|");
    // Inserting "[5]" at this offset must keep the row well-formed (still
    // starts with `|`, still ends with `|`).
    const inserted = text.slice(0, offset) + "[5]" + text.slice(offset);
    const firstLine = inserted.split("\n")[0];
    expect(firstLine.startsWith("|")).toBe(true);
    expect(firstLine.endsWith("|")).toBe(true);
  });

  it("leaves non-table lines that happen to end with '|' alone", () => {
    // Pipe in regular prose at end of sentence — not a table row.
    const text = "Use the regex foo|bar";
    // rawOffset at end → snapToSentenceEnd returns text.length; should not
    // be slid back into the "row" because the line doesn't start with `|`.
    expect(snapToSentenceEnd(text, text.length)).toBe(text.length);
  });

  it("pushes citations past the table when they land on the delimiter row", () => {
    // The delimiter row must contain ONLY pipes, dashes, colons, and
    // whitespace -- sliding a marker INTO it (``|------[3]|``) breaks the
    // table just as badly as gluing it outside. Markers in this position
    // must end up after the whole table block.
    const text =
      "| Field | Details |\n" +
      "|-------|---------|\n" +
      "| Contract # | CLT09 |\n" +
      "\nFollow-up sentence.";
    // Offset at end of the delimiter row (just after its trailing `|`)
    const delimRowEnd = text.indexOf("\n", text.indexOf("|-------"));
    const offset = snapToSentenceEnd(text, delimRowEnd);
    // Marker must land at or past the end of the last table-row line, never
    // inside the delimiter row.
    const tableEnd = text.indexOf("CLT09 |") + "CLT09 |".length;
    expect(offset).toBeGreaterThanOrEqual(tableEnd);
    // Inserting "[3]" at this offset must leave the delimiter row clean.
    const inserted = text.slice(0, offset) + "[3]" + text.slice(offset);
    const lines = inserted.split("\n");
    expect(lines[1]).toBe("|-------|---------|"); // delimiter untouched
  });
});
