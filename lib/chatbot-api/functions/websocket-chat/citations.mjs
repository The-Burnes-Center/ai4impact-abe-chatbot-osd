...
/**
 * If ``offset`` would place a citation marker immediately after the trailing
 * ``|`` of a GFM table row, slide it inside the last cell instead.
 *
 * A row like ``| Field | Details |\n`` becomes ``| Field | Details |[5]\n``
 * with naive insertion -- a non-whitespace token after the closing pipe makes
 * remark-gfm reject the row as a table, and the whole block renders as
 * literal pipe-delimited text. Moving the marker to before the pipe keeps
 * the GFM grammar intact: ``| Field | Details[5] |\n`` still parses cleanly.
 *
 * Only intervenes when the line starts with ``|`` (with optional leading
 * whitespace), so unrelated content ending in ``|`` is unaffected.
 */
export function adjustOffsetForMarkdownTable(text, offset) {
  ...
  // First non-whitespace char on the line must be `|` for this to be a row
  let i = lineStart;
  while (i < offset && (text[i] === ' ' || text[i] === '\t')) i++;
  if (text[i] !== '|') return offset;

  // Slide back over the closing pipe and any padding whitespace before it,
  // so the marker lands inside the cell rather than hugging the pipe.
  let pos = offset - 1;
