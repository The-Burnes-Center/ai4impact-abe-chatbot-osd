 ...
 *
 * After picking an offset, runs {@link adjustOffsetForMarkdownTable} so the
 * marker doesn't land glued to the closing pipe of a GFM table row, which
 * causes remark-gfm to bail out of table parsing and render the whole block
 * as inline pipe-delimited text.
 */
export function snapToSentenceEnd(text, rawOffset) {
  const len = text.length;
  if (rawOffset >= len) return adjustOffsetForMarkdownTable(text, len);
  ...
  for (let i = rawOffset; i < limit; i++) {
    ...
    if (ch === '.' || ch === '!' || ch === '?') {
      ...
      if (next === undefined || next === ' ' || next === '\n' || next === '\r') {
        return adjustOffsetForMarkdownTable(text, i + 1);
      }
    }
    if (ch === '\n') {
      return adjustOffsetForMarkdownTable(text, i);
    }
  }
  ...
  return adjustOffsetForMarkdownTable(text, pos);
}

/**
 * Markdown table-row line classifier. Only matches lines that start (after
 * optional whitespace) with ``|``.
 *
 * - ``delimiter``: GFM separator like ``|-------|:-----:|``. Must contain
 *   ONLY ``|``, ``-``, ``:``, spaces, and tabs once the leading whitespace is
 *   skipped. Inserting any non-allowed character on this line invalidates the
 *   delimiter and makes remark-gfm reject the whole block as a table.
 * - ``row``: any other table row (header or data) that starts with ``|``.
 * - ``null``: not a table line.
 */
function classifyTableLine(text, lineStart, lineEnd) {
  let i = lineStart;
...
/**
 * Keep citation markers from breaking GFM tables.
 *
 * When ``offset`` lands on the trailing ``|`` of a table row, the naive
 * insertion produces ``| Details |[5]\n``. remark-gfm refuses to parse that
 * as a table and renders the whole block as literal pipe-delimited prose.
 * Two corrections, depending on which kind of row the offset points at:
 *
 * - **Header or data row** -- slide the offset back inside the last cell so
 *   the row stays well-formed: ``| Details[5] |``.
 * - **Delimiter row** (``|-------|---------|``) -- the delimiter line must
 *   contain ONLY ``|``, ``-``, ``:``, spaces, and tabs; sliding inside it
 *   corrupts the syntax. Instead, push the offset forward past every
 *   contiguous table-row line so the marker lands immediately after the
 *   table block, where it can safely sit in the trailing paragraph.
 *
 * Lines that happen to end with ``|`` but aren't tables (e.g. a regex in
 * prose) are unaffected because they don't start with ``|``.
 */
export function adjustOffsetForMarkdownTable(text, offset) {
  ...
  let lineEnd = offset - 1;
  while (lineEnd < text.length && text[lineEnd] !== '\n') lineEnd++;

  const kind = classifyTableLine(text, lineStart, lineEnd);
  if (kind === null) return offset;

  if (kind === 'row') {
    // Slide back over the trailing pipe and any padding whitespace before
    // it so the marker lands inside the last cell.
    let pos = offset - 1;
    while (pos > 0 && (text[pos - 1] === ' ' || text[pos - 1] === '\t')) pos--;
    return pos;
  }

  // Delimiter row: advance past every contiguous table-row line so the
  // marker lands in the paragraph that follows the table.
  let pos = lineEnd;
  while (pos < text.length) {
    // pos sits on a newline -- look at the next line
    ...
    const nextKind = classifyTableLine(text, nextLineStart, nextLineEnd);
