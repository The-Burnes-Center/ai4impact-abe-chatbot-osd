...
export const STATIC_TOOLS = [
  ...
  {
    "name": "fetch_metadata",
    "description": "Retrieve the knowledge-base document inventory. By default returns a compact map of {filename: tag_category} for every document — ideal for spotting sibling documents (e.g. a CUG and its RFR share the same contract prefix) and for answering 'what's in the KB?' questions. Pass full=true only when you actually need each document's natural-language summary; the full form is large, so prefer the compact default and call retrieve_full_document for the few files you need details on.",
    "input_schema": {
      "type": "object",
      "properties": {
        "full": {
 ...
 */
export const fetchMetadata = async ({ full = false } = {}) => {
