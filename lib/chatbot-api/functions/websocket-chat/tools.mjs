...
export const STATIC_TOOLS = [
  {
    "name": "query_db",
    "description": "Query a vector database for any information in your knowledge base. Try to use specific key words when possible. When the question concerns one known contract/document, pass within_document to scope the search to that document family — prefer this over retrieve_full_document when you only need a targeted passage rather than the whole file.",
    "input_schema": {
      "type": "object",
      "properties": {
        "query": {
        ...
        "within_document": {
          "type": "string",
          "description": "Optional filename or partial filename (e.g. a contract identifier) to restrict the semantic search to one document family. Matched against source file names the same way retrieve_full_document matches. Use when the question is about one known contract/document and you need a targeted passage rather than the whole file."
        }
      },
      "required": ["query"]
    }
  },
  {
    ...
    "description": "Retrieve the COMPLETE content of a specific document from the knowledge base by filename. Use this to get an entire Contract User Guide (CUG) or policy document when you need comprehensive contract details — not just snippets. Call this when a query_db search reveals a relevant document, OR whenever the user names a specific contract, RFR, or document by its identifier (e.g. 'ITS88', 'FAC115') — you may pass the bare identifier directly, since partial filenames are matched. IMPORTANT: query_db (semantic search) frequently fails to surface a document by its contract/RFR number even when that document exists, so do NOT wait for query_db to confirm a document before retrieving it by identifier, and do NOT conclude a contract is missing without trying this tool. Returns ALL chunks of the document in page order with no truncation.",
    "input_schema": {
      "type": "object",
      "properties": {
        "document_name": {
  ...
  {
    "name": "fetch_metadata",
    "description": "Retrieve the knowledge-base document inventory. By default returns a compact map of {filename: tag_category} for every document — ideal for spotting sibling documents (e.g. a CUG and its RFR share the same contract prefix) and for answering 'what's in the KB?' questions. Pass filename_contains with a contract identifier to get just that contract family's entries instead of the whole catalog. Pass full=true only when you actually need each document's natural-language summary; the unfiltered full form is large, but combined with filename_contains it is cheap — the fastest way to see what each document in one contract family covers. Otherwise prefer the compact default and call retrieve_full_document for the few files you need details on.",
    "input_schema": {
      "type": "object",
      "properties": {
        "full": {
          "type": "boolean",
          "description": "If true, include each document's summary and tags. Defaults to false (compact filename → category map).",
          "default": false
        },
        "filename_contains": {
          "type": "string",
          "description": "Optional substring to restrict the inventory to matching filenames, e.g. a contract identifier like 'FAC115'. Matched case-insensitively. Combine with full=true to get complete summaries for just one contract family."
        },
        "reason": {
          "type": "string",
...
/**
 * Hard string-level cut for oversized results that have no `rows` array to
 * trim (e.g. a fetch_metadata inventory object). The cut payload is no longer
 * valid JSON, but the appended note makes the truncation explicit so the
 * model narrows the request instead of trusting silently incomplete data.
 */
function hardTruncateWithNote(resultStr) {
  const note = `\n\n[TRUNCATED: result was ${resultStr.length} chars, over the ` +
    `${MAX_TOOL_RESULT_CHARS}-char tool-result limit. Data above may be cut ` +
    `mid-record. Narrow the request (e.g. pass filename_contains, drop ` +
    `full=true, or use tighter filters) and retry for complete data.]`;
  return resultStr.slice(0, MAX_TOOL_RESULT_CHARS - note.length) + note;
}
 ...
 *
 * Oversized results that are not row-shaped (no non-empty `rows` array —
 * e.g. a fetch_metadata full inventory, which is a plain JSON object) fall
 * back to {@link hardTruncateWithNote}: a string-level cut with an explicit
 * truncation note rather than silently entering history uncapped.
 *
...
export function capToolResultSize(resultStr) {
  ...
  try {
    ...
    if (!Array.isArray(data.rows) || data.rows.length === 0) {
      return hardTruncateWithNote(resultStr);
    }
 ...
 *
 * @returns {Promise<Array<{index_name: string, display_name: string, description: string, columns: string[], row_count: number, date_columns: string[]|null}>>}
 *   Array of index metadata objects, one per registered Excel index.
 *   `date_columns` is `null` for legacy registry items written before
 *   date-column inference existed, otherwise an array (possibly empty).
 */
export async function loadIndexMetadata() {
  ...
  try {
    ...
    for (const item of resp.Items || []) {
      ...
      // Three-state: missing attribute (legacy item) → null; present → array
      // of column names, possibly empty (= index verified to have no dates).
      const dateColumns = Array.isArray(item.date_columns?.L)
        ? item.date_columns.L.map(c => c.S || "")
        : null;
      indexes.push({ index_name: indexName, display_name: displayName, description, columns, row_count: rowCount, date_columns: dateColumns });
    }
 ...
 *
 * @param {Array<{index_name: string, display_name: string, description: string, columns: string[], row_count: number, date_columns?: string[]|null}>} indexes
...
export function buildExcelIndexTool(indexes) {
  ...
  const indexDescriptions = indexes.map((idx, i) => {
    ...
    let line = `${i + 1}. ${idx.index_name} — ${idx.display_name} (${idx.row_count} rows).${desc} Columns: ${idx.columns.join(", ")}`;
    // Three-state: array → render date availability; null/absent (legacy
    // registry item, inference never ran) → say nothing about dates.
    if (Array.isArray(idx.date_columns)) {
      line += idx.date_columns.length > 0
        ? ` Date columns (usable with date_before/date_after/sort_by): ${idx.date_columns.join(", ")}`
...
/**
 * Fetch the knowledge base document inventory from S3 via a dedicated Lambda.
 *
 * Invokes the metadata retrieval Lambda (env `METADATA_RETRIEVAL_FUNCTION`)
 * which reads `metadata.txt` from the knowledge bucket. By default returns
 * the compact form ({filename: tag_category}); pass `{ full: true }` to get
 * each document's summary and tags. The metadata is no longer inlined into
 * the system prompt — the model retrieves it on demand via the
 * `fetch_metadata` tool, which keeps the prompt small and stays current
 * when the KB is re-synced mid-session.
 *
 * @param {object} [options]
 * @param {boolean} [options.full=false] - Return full summaries/tags if true.
 * @param {string|null} [options.filenameContains=null] - Optional substring;
 *   when set, the Lambda restricts the inventory to filenames containing it
 *   (case-insensitive), e.g. a contract identifier like "FAC115".
 * @returns {Promise<object|null>} The metadata object, or `null` on error.
 */
export const fetchMetadata = async ({ full = false, filenameContains = null } = {}) => {
...
export function excelQueryNeedsEntitySummaryEnrichment(query) {
  ...
  if (typeof query.free_text === "string" && query.free_text.trim().length > 0) return true;
  if (query.filters && typeof query.filters === "object" && !Array.isArray(query.filters)
      && Object.keys(query.filters).length > 0) return true;
  return false;
}

// Above this many entity groups, omit the per-entity maps from
// `_entity_summary` (keep only the distinct count). The summary is injected
// before capToolResultSize, which only trims `rows` -- an unbounded map could
// dominate or blow the 60K tool-result cap.
const MAX_ENTITY_BREAKDOWN_GROUPS = 50;

/**
 * Enrich a filtered Excel query result with entity-level summary stats.
 *
 ...
 *
 *  - `row_total_matches` -- total rows matching the filters.
 ...
 *
 * Applies to date-filtered, `free_text`, and `filters` queries alike. The
 * enrichment is skipped when the original query already uses aggregation
 * operators (`count_unique`, `group_by`, `distinct_values`), when no entity
 * ID column can be identified in the index metadata, or when 0-1 rows
 * matched (the row-vs-entity distinction is meaningless there, so the
 * second query would be wasted).
 *
 * When more than {@link MAX_ENTITY_BREAKDOWN_GROUPS} entity groups come
 * back, the per-entity maps are omitted (only `distinct_entity_count` is
 * kept) -- `_entity_summary` is injected *before* {@link capToolResultSize},
 * which only trims `rows`, so a huge map could dominate or blow the 60K cap.
 *
...
export async function enrichExcelIndexResult(query, indexName, toolResultStr, idxMeta) {
  ...
  if (parsed.total_matches <= 1) return toolResultStr;
  ...
  const groups = enrichParsed.groups || {};
  const groupCount = Object.keys(groups).length;
  const includeBreakdown = groupCount <= MAX_ENTITY_BREAKDOWN_GROUPS;
  parsed._entity_summary = {
    ...
    ...(includeBreakdown
      ? { rows_per_entity: groups }
      : {
          breakdown_note: `entity breakdown omitted (${groupCount} entities); use group_by for a full breakdown`,
        }),
    ...(includeBreakdown && enrichParsed.group_max_values
      ? {
 ...
 *
 * Steps:
 *  1. Formats the current date in US Eastern time for injection into the prompt.
 *  2. Renders the prompt template (from prompt-registry) with the date,
 *     producing the final prompt text along with version/hash identifiers used
 *     for Bedrock prompt caching.
 *
 * The document inventory is no longer assembled here — the model fetches it on
 * demand via the `fetch_metadata` tool, which keeps the system prompt small
 * (~14 KB vs ~200 KB previously) and stays current when the KB changes.
 *
...
export async function constructSysPrompt() {
  ...
  const rendered = await loadRenderedPrompt(PROMPT, dateStr);
  return {
