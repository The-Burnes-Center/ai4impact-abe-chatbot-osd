...
export default class ClaudeModel{
   ...
   * @param {AbortSignal} [abortSignal] - Cancels the Bedrock stream (and token
   *   generation) when the client disconnects mid-response.
   * @returns {Promise<AsyncIterable>} Bedrock streaming response body.
   */
  async getStreamedResponse(system, history, tools, abortSignal) {
    ...
    // Forward the caller's abort signal so a client disconnect cancels the
    // Bedrock inference (stops token generation and cost), not just our reads.
    const apiResponse = await this.client.send(command, abortSignal ? { abortSignal } : {});
  ...
  async getResponse(system, history, message, { maxTokens = DEFAULT_MAX_OUTPUT_TOKENS } = {}) {
    ...
    try {
      ...
      // Bedrock may return an empty content array (e.g. guardrail intervention,
      // model refusal, or stop_reason that isn't text). Guard against undefined
      // before reading `.text` so we surface a clean fallback instead of crashing.
      const block = Array.isArray(parsed?.content) ? parsed.content[0] : undefined;
      const text = typeof block?.text === "string" ? block.text : "";
      if (!text) {
        console.error(
          "Bedrock response had no text content:",
          JSON.stringify({
            stop_reason: parsed?.stop_reason,
            stop_sequence: parsed?.stop_sequence,
            contentTypes: Array.isArray(parsed?.content) ? parsed.content.map(b => b?.type) : null,
            usage: parsed?.usage,
          })
        );
